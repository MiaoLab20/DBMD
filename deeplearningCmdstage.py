...
df = pd.read_csv("cmd.mdout", names=column_names, header=0, delimiter="\t")

steps = df.Steps.to_list()
steps = np.array(steps[-100000:])
steps = np.repeat(steps, 5, axis=-1)
nbsteps = len(steps)

dihedralEnergy = df.PeriodicTorsionForce.to_list()
dihedralEnergy = np.array(dihedralEnergy[-100000:]) / 4.184
dihedralEnergy = np.repeat(dihedralEnergy, 5, axis=-1)
min_dihedral, max_dihedral = min(dihedralEnergy), max(dihedralEnergy)
totalEnergy = df.PotentialEnergy.to_list()
totalEnergy = np.array(totalEnergy[-100000:]) / 4.184
totalEnergy = np.repeat(totalEnergy, 5, axis=-1)
...
isExist = os.path.exists("cmd-boosts")
if not isExist:
    os.makedirs("cmd-boosts")
...
plt.savefig("cmd-boosts/boostdistr0.png")

iter = 0
while (anharmdual > 1e-2) and iter <= 5:
    ...
    isExist = os.path.exists("cmd-cptot")
    if not isExist:
        os.makedirs("cmd-cptot")
    isExist = os.path.exists("cmd-cpdih")
    if not isExist:
        os.makedirs("cmd-cpdih")

    cptot_path = "cmd-cptot/cptot-{epoch:04d}.ckpt"
    cpdih_path = "cmd-cpdih/cpdih-{epoch:04d}.ckpt"
    ...
    totadjustboost, dihadjustboost = 0, 0
    ...
    if anharmdual <= 1e-2:
        ...
        plt.xticks(fontsize=12)
        plt.yticks(fontsize=12)
        plt.savefig("cmd-boosts/boostdistr" + str(iter + 1) + ".png")
        ...
        plt.yticks(fontsize=12)
        plt.legend()
        plt.savefig("cmd-boosts/fconstants0" + str(iter + 1) + ".png")
        ...
        plt.yticks(fontsize=12)
        # plt.legend()
        plt.savefig("cmd-boosts/etotal" + str(iter + 1) + ".png")
        ...
        plt.yticks(fontsize=12)
        # plt.legend()
        plt.savefig("cmd-boosts/edihedral" + str(iter + 1) + ".png")

        isExist = os.path.exists("cmd-restart.dat")
        if isExist:
            os.remove("cmd-restart.dat")
        cmdRestart = open("cmd-restart.dat", "w")
        cmdRestart.write("#Parameters\tValues(kcal/mol)\n")
        cmdRestart.write("(0)Trained Steps:\t" + str(nbsteps) + "\n")
        cmdRestart.write("(1)Boosted VfinalD:\t" + str(Xdih[-1]) + "\n")
        cmdRestart.write("(2)Boosted VminD:\t" + str(min(dihedralEnergy[-10000:])) + "\n")
        cmdRestart.write("(3)Boosted VmaxD:\t" + str(max(dihedralEnergy[-10000:])) + "\n")
        cmdRestart.write("(4)Maximum k0D:\t" + str(max(dihedralfc0s)) + "\n")
        cmdRestart.write("(5)Average k0D:\t" + str(np.mean([k0D for k0D in dihedralfc0s if np.isnan(k0D) == False])) + "\n")
        cmdRestart.write("(6)Boosted VfinalP:\t" + str(Xtot[-1]) + "\n")
        cmdRestart.write("(7)Boosted VminP:\t" + str(min(totalEnergy[-10000:])) + "\n")
        cmdRestart.write("(8)Boosted VmaxP:\t" + str(max(totalEnergy[-10000:])) + "\n")
        cmdRestart.write("(9)Maximum k0P:\t" + str(max(totalfc0s)) + "\n")
        cmdRestart.write("(10)Average k0P:\t" + str(np.mean([k0P for k0P in totalfc0s if np.isnan(k0P) == False])) + "\n")
        cmdRestart.close()
